priceN <- function(xts, x = "close", n = 1:10, trading_days = 50, spread = TRUE){
        ...
        price_columns = paste0("price_plus_", n)
                        ...
                        wrapr::let(c(col_x_name = x),
                                   xts %>%
                                           ...
                                           dplyr::mutate(symbol = symbol, 
                                                         key = price_columns[i],
                                                         value = dplyr::lead(col_x_name, n[i])) %>%
        ...
        if(spread){
                tempDf = tempList %>%
                        ...
                        dplyr::select(symbol, date, price = close, everything())
        } else {
                tempDf = tempList %>%
                        dplyr::bind_rows() %>%
                        dplyr::select(symbol, date, price = close, everything())
