output2_transformed <- output2 %>%
        ...
        dplyr::mutate(
                ...
                # sell if,
                message_s1 = case_when(macd_flag == -1 & close < zlema & (close < evwma | close < chanExit_long) ~ "sell - macd",
                                       ha_flag == -1 & close < zlema & (close < evwma | close < chanExit_long) ~ "sell - ha",
                                       evwma_flag == -1 & close < zlema ~ "sell - evwma",
                                       TRUE ~ message_b),
                
                message_s2 = case_when(percent_change_lag5_day <0 &
                                       ...
                                       #candle_stick_pattern == -1 & close < zlema ~ "sell - candlestick",
                                       TRUE ~ message_b),
...
poc <- output2_transformed %>%
        ...
        dplyr::mutate(
                ...
                message_s4 = case_when(close < trailing_stop_loss_yesterday ~ "sell - profit protect", 
                                       TRUE ~ message_s4),
                # stop-loss
                #message_s1 = case_when(close < hard_stop_down_limit ~ "sell - stop loss (hard)", TRUE ~ message_s1),
                message_s2 = case_when(close < hard_stop_down_limit | close > hard_stop_up_limit ~ "sell - stop loss (hard)", TRUE ~ message_s2)
                #message_s3 = case_when(close < hard_stop_limit ~ "sell - stop loss (hard)", TRUE ~ message_s3)
                #message_s4 = case_when(close < hard_stop_limit ~ "sell - stop loss (hard)", TRUE ~ message_s4)
        ) %>%
        arrange(symbol, date)
